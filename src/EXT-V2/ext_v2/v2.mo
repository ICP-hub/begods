...
actor class EXTNFT(init_owner : Principal) = this {
  ...
  let ENTREPOT_SALE_FEES_ADDRESS : AccountIdentifier = "492aacd082a520dfc078678759756ef4ac11133083799c77d4acc7a3490cb5ec";
  let ENTREPOT_SALE_FEES_AMOUNT : Nat64 = 6000;
  let ENTREPOT_MARKETPLACE_FEES_ADDRESS : AccountIdentifier = "492aacd082a520dfc078678759756ef4ac11133083799c77d4acc7a3490cb5ec";
  let ENTREPOT_MARKETPLACE_FEES_AMOUNT : Nat64 = 1000;
  let HTTP_NOT_FOUND : HttpResponse = {
    status_code = 404;
    headers = [];
    body = Blob.fromArray([]);
  ...
  //Services
  let ExternalService_Cap = Cap.Cap(?"cuj6u-c4aaa-aaaaa-qaajq-cai", cap_rootBucketId);
  let ExternalService_ICPLedger = actor "cuj6u-c4aaa-aaaaa-qaajq-cai" : actor {
  ...
  func _getFreeAssetCanister(size : Nat) : ?(Principal, Nat) {
    Array.find<(Principal, Nat)>(
      Array.sort<(Principal, Nat)>(
        Iter.toArray(_assetCanisters.entries()),
        func(a : (Principal, Nat), b : (Principal, Nat)) : Order {
          if (a.1 < b.1) {
            return #less;
          };
          if (a.1 > b.1) {
            ...
            switch (settlement.purchase) {
              case (#sale quantity) {
                ...
                if (Nat64.toNat(quantity) > availableTokens()) {
                  ...
                  data_saleTransactions := Array.append(data_saleTransactions, [{ tokens = tokens; seller = Principal.fromActor(this); price = settlement.amount; buyer = settlement.payer; time = Time.now() }]);
    ...
    switch (data_saleCurrent) {
      case (?cs) {
        ...
        for (group in cs.groups.vals()) {
          ...
          if (add) {
            groups := Array.append(groups, [{ id = groupId; name = group.name; start = group.start; end = group.end; available = _checkSaleLimits(groupId, address, 1, group.limit); pricing = group.pricing }]);
          };
          groupId += 1;
        };
        return ?{
  ...
  func _checkSaleLimits(groupId : Nat, address : AccountIdentifier, quantity : Nat64, limit : (Nat64, Nat64)) : Bool {
    if (limit.0 > 0) {
      let purchased = Array.foldLeft(
        Array.mapFilter(
          data_saleSoldTracker,
          func(a : (Nat, AccountIdentifier, Nat64)) : ?Nat64 {
        ...
        0 : Nat64,
        func(a : Nat64, b : Nat64) : Nat64 { a + b },
      );
  ...
  func _ext_httpHome() : HttpResponse {
    ...
    return {
      ...
      body = Text.encodeUtf8(
        ...
        "---\n" #
        "Royalty Address:                          " # debug_show (
          Array.map<(AccountIdentifier, Nat64), AccountIdentifier>(
            config_royalty,
            func(cr : (AccountIdentifier, Nat64)) : AccountIdentifier {
              return cr.0;
            },
          )
        ) # "\n" #
        "Royalty Amount:                           " # debug_show (
          Array.map<(AccountIdentifier, Nat64), Nat64>(
            config_royalty,
            func(cr : (AccountIdentifier, Nat64)) : Nat64 {
              return cr.1;
            },
          )
        ) # "\n" #
  ...
  func ext_checkPayment(paymentaddress : AccountIdentifier) : async ?(Payment, Result.Result<ICPTs, Text>) {
    switch (_paymentSettlements.get(paymentaddress)) {
      case (?settlement) {
        let response : ICPTs = await ExternalService_ICPLedger.account_balance_dfx({
  ...
  public query func getMetadata() : async [(TokenIndex, MetadataLegacy)] {
    Iter.toArray(
      HashMap.map<TokenIndex, Metadata, MetadataLegacy>(
        ...
        func(a : (TokenIndex, Metadata)) : MetadataLegacy {
          _convertToLegacyMetadata(a.1);
        },
      ).entries()
    );
  };
  public query func getAllFungibleTokenData() : async [(TokenIndex, AccountIdentifier, Metadata)] {
    if (_tokenMetadata.size() == 0) {
      return [];
    };

    let fungibleTokenData = Buffer.Buffer<(TokenIndex, AccountIdentifier, Metadata)>(_tokenMetadata.size());
    ...
    for ((tokenIndex, metadata) in _tokenMetadata.entries()) {
      switch (metadata) {
        case (#fungible(_)) {
          ...
          fungibleTokenData.add((tokenIndex, owner, metadata));
        };
        case (#nonfungible(_)) {
